...
{
public:
  explicit RelayTimer(const rclcpp::NodeOptions & options) : TildeNode("relay_timer", options)
  {
    ...
    auto timer_callback = [this, proc_dur, update_stamp]() -> void {
      ...
      if (msg_pc_) {
        ...
        RCLCPP_INFO(
          ...
          msg_pc_->header.frame_id.c_str(), nanoseconds(msg_pc_->header.stamp));
      ...
      if (msg_str_) {
        ...
        RCLCPP_INFO(this->get_logger(), "RelayTimer pub String seq: %s", msg_str_->data.c_str());
